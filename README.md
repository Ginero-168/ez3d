...
ถ้าต้องส่งงานข้ามเครื่องหรือข้ามเบราว์เซอร์ ให้ใช้ปุ่ม **ส่งออก ZIP** เพื่อสร้างไฟล์ `.ez3d.zip` ที่รวม `project.json` และไฟล์โมเดล 3D ที่เกี่ยวข้องไว้ในแพ็กเดียว

การอัปโหลดโมเดล 3D รองรับไฟล์เดี่ยว เช่น `.glb`, `.fbx`, `.stl`, `.ply`, `.dae` และรองรับการเลือกหลายไฟล์พร้อมกันสำหรับ asset set เช่น `.gltf + .bin + textures` หรือ `.obj + .mtl + textures`
